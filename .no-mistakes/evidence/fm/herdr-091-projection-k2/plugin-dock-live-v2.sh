...
[ "$TAG" = base ] && { say "summary"; echo "failures: $FAILS"; exit "$FAILS"; }

say "S3 husk respawn: create_task with the same label over the agent-less flat1 tab replaces it; new tab is exactly one pane"
if [ -n "${FPANE:-}" ]; then
  ...
  echo "create_task rc=$rc out=$OUT"
  if [ $rc = 0 ]; then
    NTAB=${OUT%% *}; NPANE=${OUT##* }; sleep 2
    echo "old tab panes: $(tabpanes "$FWS" "$FTAB")"; P=$(tabpanes "$FWS" "$NTAB"); echo "new tab panes: $P"
    printf '%s' "$P" | jq -e --arg p "$NPANE" 'length == 1 and .[0].pane_id == $p' >/dev/null && ! lab pane get "$FPANE" >/dev/null 2>&1 \
      && ok "husk replaced; replacement tab holds exactly its task pane" || bad "respawn shape"
    FTAB=$NTAB; FPANE=$NPANE
  else bad "respawn over husk refused"; fi
fi

say "S4 captain focuses the flat task tab: does herdr-sidebar re-dock, and do discovery/respawn still work?"
if [ -n "${FPANE:-}" ]; then
  lab tab focus "$FTAB" >/dev/null; sleep 3
  P=$(tabpanes "$FWS" "$FTAB"); echo "tab panes after focus: $P"
  if printf '%s' "$P" | jq -e 'length > 1' >/dev/null; then
    echo "NOTE: plugin re-docked after focus (outside create-time settle)"
    LIVE=$(adapter fm_backend_herdr_list_live "$LAB"); echo "list_live: ${LIVE:-<empty>}"
    printf '%s\n' "$LIVE" | grep -qF "$LAB:$FPANE" && ok "list_live still resolves after re-dock" || bad "list_live skips flat1 after focus re-dock"
    R=$(adapter fm_backend_herdr_resolve_bare_selector fm-flat1 2>&1); echo "resolve_bare_selector: $R"
    [ "$R" = "$LAB:$FPANE" ] && ok "resolve after re-dock" || bad "resolve_bare_selector fails after focus re-dock"
    OUT=$(adapter fm_backend_herdr_create_task "$LAB:$FWS" fm-flat1 "$PROJ" "" 2>&1); rc=$?
    echo "respawn over re-docked husk: rc=$rc out=$OUT"
    if [ $rc = 0 ]; then ok "respawn over re-docked husk"; FTAB=${OUT%% *}; FPANE=${OUT##* }; else bad "respawn over re-docked husk refused"; fi
  else ok "no re-dock on focus"; fi
fi

say "S5 adversarial: an unregistered captain split in the task tab is never closed by the plugin prune/settle"
if [ -n "${FPANE:-}" ]; then
  SPANE=$(lab pane split "$FPANE" --direction right --no-focus | jq -r '.result.pane.pane_id // empty'); sleep 2
  echo "split pane: $SPANE; tab panes: $(tabpanes "$FWS" "$FTAB")"
  adapter fm_backend_herdr_tab_settle_plugin_panes "$LAB" "$FWS" "$FTAB" "$FPANE"; rc=$?
  echo "settle rc=$rc; tab panes after settle: $(tabpanes "$FWS" "$FTAB")"
  [ $rc != 0 ] && lab pane get "$SPANE" >/dev/null 2>&1 && ok "settle refused to converge and the captain split $SPANE survived" || bad "split closed or settle claimed convergence"
  echo "direct probe: $(lab plugin pane close "$SPANE" 2>&1)"
  lab pane get "$SPANE" >/dev/null 2>&1 && ok "Herdr refused plugin pane close on the split" || bad "split gone after probe"
  PDBG=$(lab pane list --workspace "$FWS" | jq -c '[.result.panes[] | .pane_id]'); echo "workspace $FWS panes before flat teardown: $PDBG"
fi

say "S6 teardown: projected workspace removed; flat task pane closed; no plugin-only tab left behind"
if [ -n "${PWS:-}" ]; then
  lab tab focus "$PTAB" >/dev/null; sleep 2; echo "projected ws after focus: $(wspanes "$PWS")"
  teardown tangent-geom-s1 > "$W/pt.out" 2> "$W/pt.err" || echo "teardown rc!=0: $(tail -3 "$W/pt.err")"; sleep 1
  lab workspace get "$PWS" >/dev/null 2>&1 && bad "projected workspace survived: $(wspanes "$PWS")" || ok "projected workspace removed"
fi
if [ -n "${FPANE:-}" ]; then
  teardown flat1 > "$W/ft.out" 2> "$W/ft.err" || echo "teardown rc!=0: $(tail -3 "$W/ft.err")"; sleep 1
  # the S3 replacement pane is not in meta (create_task was driven directly), so kill it through the adapter
  adapter fm_backend_herdr_kill "$LAB:$FPANE"; sleep 1
  lab pane get "$FPANE" >/dev/null 2>&1 && bad "flat task pane alive" || ok "flat task pane closed"
  echo "workspace $FWS after flat teardown: $(lab pane list --workspace "$FWS" 2>&1 | jq -c '[.result.panes[]? | {pane_id: .pane_id, tab_id: .tab_id, label: (.label // null), plugin_tokens: ((.tokens // {}) | keys)}] // .' 2>/dev/null)"
  [ -n "${SPANE:-}" ] && { lab pane get "$SPANE" >/dev/null 2>&1 && ok "captain split survived task teardown" || echo "NOTE: captain split gone after teardown"; }
fi
echo "workspaces now: $(lab workspace list | jq -c '[.result.workspaces[] | {id: .workspace_id, label: .label}]')"
