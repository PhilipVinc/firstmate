...
`bin/backends/herdr.sh` owns the mechanics in `fm_backend_herdr_tab_prune_plugin_panes`, and `tests/fm-backend-herdr.test.sh` pins the docked, flat and projected late-docked, and unregistered-split shapes, plus flat recovery and husk respawn past a docking or re-docking plugin, against its stateful fake.
